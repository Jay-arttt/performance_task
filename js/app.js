...
function openCardMenu(btn, task, sheetName) {
  ...
  const bgColor = window.matchMedia('(prefers-color-scheme: dark)').matches ? '#242228' : '#ffffff';
  menu.style.cssText = `position:fixed;top:${top}px;left:${left}px;z-index:9999;background:${bgColor};border:.5px solid rgba(0,0,0,0.15);border-radius:10px;padding:4px;min-width:${menuW}px;box-shadow:0 8px 24px rgba(0,0,0,.18);`;
...
function renderFlow() {
  ...
  renderFlowControls();
  document.getElementById('memberFilterWrap').innerHTML = ''; // 다른 탭 필터 잔재 제거
