...
function buildBulkModalHTML() {
  ...
  const stepRows = FLOW_STEPS_BULK.map((step, i) => {
    ...
    const date = i === 0 ? today : '';
    return `<div class="bulk-step-row" data-step="${step}" data-idx="${i}">
      <span class="bulk-step-badge" style="background:${stepStyle.bg};color:${stepStyle.c};">${step}</span>
      <span style="font-size:11px;color:var(--color-text-tertiary);">→</span>
      <input type="date" class="field-input bulk-date" style="flex:1;font-size:12px;padding:5px 8px;" value="${date}" data-idx="${i}">
      <div class="assignee-picker bulk-assignee" data-idx="${i}" style="flex-shrink:0;">
        ${memberChips()}
      </div>
    </div>`;
  }).join('');

  return `<div class="modal-box" style="max-width:540px;">
    ...
    <div class="modal-body" style="gap:14px;">
      ...
      <div class="field-group">
        <label class="field-label">단계별 일정 <span style="font-size:10px;color:var(--color-text-tertiary);font-weight:400;">— 날짜 변경 시 이후 단계 자동 업데이트</span></label>
        <div style="display:flex;flex-direction:column;gap:7px;" id="bulkStepRows">
          ${stepRows}
        </div>
...
function initBulkModal() {
  ...
  document.getElementById('bulkDefaultAssignee').addEventListener('click', e => {
    ...
    document.querySelectorAll('.bulk-assignee').forEach(picker => {
      picker.querySelectorAll('.assignee-chip').forEach(chip => {
        ...
        chip.classList.toggle('checked', isSelected);
  ...
  document.getElementById('bulkStepRows').addEventListener('change', e => {
    if (!e.target.classList.contains('bulk-date')) return;
    const idx = Number(e.target.dataset.idx);
    const allDates = [...document.querySelectorAll('.bulk-date')];
    for (let i = idx + 1; i < allDates.length; i++) {
      allDates[i].value = nextDay(allDates[i - 1].value);
    }
  });
}
...
function updateBulkCount() {
  ...
  if (mediaCount === 0) {
    ...
    label.innerHTML = `저장 시 카드 <span style="color:var(--color-text-success);font-weight:500;">${cardCount}개</span> 생성 (매체 ${mediaCount}개 × ${FLOW_STEPS_BULK.length + 1}단계)`;
  }
}
...
async function submitBulk() {
  ...
  const dates = [...document.querySelectorAll('.bulk-date')].map(i => i.value);

  if (!baseTitle) { showToast('업무명을 입력해주세요'); return; }
  if (!selectedMedia.length) { showToast('매체를 하나 이상 선택해주세요'); return; }
  if (dates.some(d => !d)) { showToast('모든 단계의 기한을 입력해주세요'); return; }
...
document.addEventListener('click', e => {
  ...
  if (hidden) {
    hidden.value = [...picker.querySelectorAll('input[type=checkbox]:checked')].map(c => c.value).join(',');
  }
});
